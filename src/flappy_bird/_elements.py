"""Here are contained the classes of the elements that appear on the game"""
import random
...
class Bird:
    ...
    def __init__(self, x_pos: float = 200, y_pos: float = 350):
        """
        Inits the class
    ...
    def on_the_ground(self) -> bool:
        return self.y_pos + self.img.get_height() >= 725
...
class Base:
    ...
    VEL = Pipe.VEL
    WIDTH = cte.BASE_IMG.get_width()

    def __init__(self, y_pos: int = 730):
        """
        Inits the class.
    ...
    def draw(self, window: Union[pygame.Surface, pygame.SurfaceType]):
        """Draws the top & bottom pipes at the window"""
        window.blit(cte.BASE_IMG, (self.x_pos_1, self.y_pos))
        window.blit(cte.BASE_IMG, (self.x_pos_2, self.y_pos))
